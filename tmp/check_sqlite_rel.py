...
LOG_FILE = "debug_sc.txt"

with open(LOG_FILE, "w") as f:
    f.write(f"Checking DB at: {os.path.abspath(DB_PATH)}\n")
    if not os.path.exists(DB_PATH):
        ...
        try:
            ...
            f.write("\nRecent docs:\n")
            cursor.execute("SELECT number, sale_condition_id FROM documents WHERE (created_at IS NOT NULL) ORDER BY created_at DESC LIMIT 5")
