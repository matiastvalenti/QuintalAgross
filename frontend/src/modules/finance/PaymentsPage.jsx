...
import { 
  ...
  Eye, Trash2, X, Download, TrendingUp, Clock, CreditCard, Calendar, User, DollarSign, ArrowRight
} from 'lucide-react';
...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("payment-changed", onRefresh);
    ...
    return () => {
      ...
      window.removeEventListener("payment-changed", onRefresh);
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      const [paymentsRes, entitiesRes] = await Promise.all([
        api.get('/accounting/documents/'),
        api.get('/entities/?type=supplier')
      ]);
      
      const filteredDocs = paymentsRes.filter(d => d.doc_type === 'PAYMENT');
      setPayments(filteredDocs);
  ...
  }, [payments, search, selectedEntityId, dateFrom, dateTo, sortBy, sortDir, entities]);
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'CLOSED': return <Badge variant="success">Pagado</Badge>;
      case 'CANCELLED': return <Badge variant="error" icon={<X size={12}/>}>Anulado</Badge>;
      ...
      <div style={{ flex: 1, padding: '16px 24px 24px 24px', display: 'flex', flexDirection: 'column', gap: 16, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
            <Card style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div>
                    <div style={{ fontSize: 12, color: 'var(--text-secondary)', fontWeight: 500 }}>Total Emitidos</div>
                    <div style={{ fontSize: 20, fontWeight: 700 }}>{filtered.length}</div>
                </div>
            </Card>
            <Card style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ background: 'var(--bad-light)', color: 'var(--bad)', padding: 12, borderRadius: 12 }}>
            ...
            <Card style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div>
                    <div style={{ fontSize: 12, color: 'var(--text-secondary)', fontWeight: 500 }}>Pagos Pendientes</div>
                    <div style={{ fontSize: 20, fontWeight: 700 }}>
            ...
            <Card style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: 16 }}>
                ...
                <div>
                    <div style={{ fontSize: 12, color: 'var(--text-secondary)', fontWeight: 500 }}>Pagos en Dólares</div>
                    <div style={{ fontSize: 20, fontWeight: 700 }}>
          ...
          {showFilters && (
            <div style={{ 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)' }}>Proveedor</label>
                    <select 
          ...
          <div className={t.container} style={{ flex: 1, overflowY: 'auto' }}>
            {loading ? (
              ...
              <EmptyState 
                ...
                description={search ? "No se encontraron resultados para tu búsqueda" : "Todavía no se registraron órdenes de pago."}
                actionLabel={!search ? "Crear Primera OP" : null}
