...
export default function InvoicesPage() {
  ...
  const handleBulkEmail = async () => {
    ...
    try {
        ...
        if (err === 0) {
            showToast(`Se enviaron ${ok} facturas correctamente.`, 'success');
        } else {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const filteredDocs = data.filter((d) => d.doc_type === "INVOICE");
  ...
  return (
    <>
      <DocumentListPage 
        ...
        breadcrumbs={[{ label: 'Suite Comercial' }, { label: 'Facturación' }]}
        kpis={kpis}
        toolbar={toolbar}
        table={table}
