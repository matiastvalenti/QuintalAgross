...
import DebitNoteCreationModal from "./DebitNoteCreationModal";
...
export default function DebitNotesPage() {
  ...
  const handleBulkEmail = async () => {
    ...
    try {
        ...
        if (err === 0) {
            showToast(`Se enviaron ${ok} nota de débitos correctamente.`, 'success');
        } else {
  ...
  const handleOpenDetail = (id, number) => {
    openStandaloneWindow(`/standalone/notas-debito/${id}`);
  };
  ...
  const kpis = [
    ...
    },
    {
      ...
      subtitle: `${stats.pendingCount} nota de débitos abiertas`,
      icon: Clock,
      type: "warning"
    },
    {
    ...
    body: loading ? (
      ...
      <tr>
        <td colSpan="8">
          <EmptyState 
              icon={Search} 
              title={hasActiveFilters ? "Sin resultados" : "Sin nota de débitos registradas"}
              description={hasActiveFilters ? "Probá ajustando los filtros de búsqueda." : "Comenzá creando tu primera nota de débito de venta."}
              actionLabel={!hasActiveFilters ? "Nueva Nota de Débito" : null}
        ...
        <tr key={inv.id} className={`${s.row} ${selectedInvoices.includes(inv.id) ? s.rowSelected : ""}`} onClick={() => handleOpenDetail(inv.id, inv.number)}>
          ...
          <td className={s.td} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              <button 
                onClick={(e) => { e.stopPropagation(); window.open(`/standalone/notas-debito/nueva?nota de débito_id=${inv.id}`, `nota-debito-nota de débito-${inv.id}`, 'width=1280,height=820,left=100,top=100'); }} 
              ...
              {inv.status !== 'CANCELLED' && (
                <button onClick={(e) => handleAnnul(e, inv.id, inv.number)} style={{ all: 'unset', cursor: 'pointer', color: '#ef4444', opacity: 0.7 }} title="Anular Nota de Débito">
                  <Trash2 size={18} />
                </button>
              )}
  ...
  return (
    <>
      <DocumentListPage 
        title="Libro de Notas de Débito"
        breadcrumbs={[{ label: 'Suite Comercial' }, { label: 'Notas de Débito' }]}
        kpis={kpis}
        toolbar={toolbar}
        table={table}
