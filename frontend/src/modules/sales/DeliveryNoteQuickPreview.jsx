...
import { PackageOpen, Printer, ArrowUpRight, ArrowRight, Truck, Info, Calendar, User, FileText, FileSignature } from 'lucide-react';
...
export default function DeliveryNoteQuickPreview({ detail, entities, warehouses, onOpenFull, onPrint }) {
  ...
  const getEquivalencia = (line) => {
    ...
    if (factor && factor !== 1) {
      const eq = qty / factor;
      return `${eq.toLocaleString('es-AR', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} env.`;
    }
    return '-';
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, fontSize: 12, color: 'var(--muted)', fontWeight: 500, flexWrap: 'wrap' }}>
              ...
              {detail.origin_reference && detail.origin_reference !== 'DIRECTO' && (
                <>
                  <span>·</span>
                  <span style={{ color: 'var(--primary)' }}>OV: <strong>{detail.origin_reference}</strong></span>
                </>
              )}
              
              <span>·</span>
              <span style={{ color: 'var(--muted)', fontSize: 11 }}>
                Transporte: <strong style={{ color: 'var(--text)' }}>{detail.transport_company || 'No asignado'}</strong>
                {detail.driver_name && ` · Chofer: ${detail.driver_name} ${detail.vehicle_plate ? `(${detail.vehicle_plate})` : ''}`}
              </span>

              {detail.created_by_name && (
                <>
                  <span>·</span>
                  <span>Usuario: <strong style={{ color: 'var(--text)' }}>{detail.created_by_name}</strong></span>
                </>
              )}
      ...
      {/* Items Table Container (Scrollable only if needed) */}
        ...
        <table className={t.table} style={{ width: '100%', margin: 0 }}>
          <thead style={{ position: 'sticky', top: 0, zIndex: 1, background: 'var(--bg-page)', fontSize: 11 }}>
            <tr>
              ...
              <th className={t.th} style={{ textAlign: 'center' }}>PED/REM/PTE</th>
            </tr>
          </thead>
          <tbody>
            {(detail.lines || []).map((line, idx) => {
              ...
              const isCross = !!detail.origin_reference && detail.origin_reference !== 'DIRECTO';
              ...
              return (
                <tr key={idx} className={t.row} style={{ fontSize: 12 }}>
                  ...
                  <td className={t.td} style={{ textAlign: 'center', color: 'var(--muted)', fontSize: 11 }}>
                    {isCross ? `${qtyOrdered} / ${qtyDelivered} / ${Math.max(0, qtyOrdered - qtyDelivered)}` : '-'}
                  </td>
                </tr>
              );
            })}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: 16, borderTop: '1px solid var(--border-color)' }}>
        <div style={{ display: 'flex', gap: 32, fontSize: 12 }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 800 }}>{totalQty.toLocaleString('es-AR')}</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
