...
export default function SalesOrderForm(props) {
  ...
  useEffect(() => {
    console.time('5. Montaje SalesOrderForm');
    return () => console.timeEnd('5. Montaje SalesOrderForm');
  }, []);
  ...
  useEffect(() => {
    if (mode === "new") {
      if (initialData?.entity_id) {
        ...
        fetch(`${API_URL}/entities/${initialData.entity_id}`, { headers: { Authorization: `Bearer ${token}` }})
          .then(r => r.json())
          .then(setEntity)
      ...
      if (initialData?.salesperson_id) {
        setSalespersonId(initialData.salesperson_id);
      }
  ...
  const progress = useMemo(() => {
    ...
    const delivered = items.reduce((acc, i) => acc + (parseFloat(i.qty_delivered) || 0), 0);
    const invoiced = items.reduce((acc, i) => acc + (parseFloat(i.qty_invoiced) || 0), 0);
    ...
    const paidSum = invoices.reduce((acc, inv) => {
      ...
      if (inv.status === 'CLOSED' || inv.status === 'PAID') return acc + (inv.total_amount || 0);
    ...
    if (invoices.length > 0 && invoices.every(inv => inv.status === 'CLOSED' || inv.status === 'PAID')) {
      paidPct = 100;
    }
  ...
  const deliveredLts = useMemo(() => items.reduce((acc, item) => acc + (parseFloat(item.qty_delivered) || 0), 0), [items]);
  ...
  const hasProgress = useMemo(() => {
    return items.some(i => (parseFloat(i.qty_delivered) || 0) > 0 || (parseFloat(i.qty_invoiced) || 0) > 0);
  }, [items]);

  const projectedStatus = useMemo(() => {
    if (!linkManagerDoc) return status;
    const isDN = linkManagerDoc.type === 'REMITO';
    let totalDelivered = 0, totalInvoiced = 0, totalOrdered = 0;
    
    items.forEach(item => {
      const lineLink = linkManagerDoc.lines.find(l => l.source_sales_line_id === item.id);
      let simD = parseFloat(item.qty_delivered || 0);
      let simI = parseFloat(item.qty_invoiced || 0);
      if (lineLink) {
        if (isDN) simD = simD - parseFloat(lineLink.originalQty) + parseFloat(lineLink.newQty);
        else simI = simI - parseFloat(lineLink.originalQty) + parseFloat(lineLink.newQty);
      }
    ...
    const dPct = totalDelivered / totalOrdered, iPct = totalInvoiced / totalOrdered;
    const isDT = dPct >= 0.999, isDP = dPct > 0.001 && !isDT;
    const isIT = iPct >= 0.999, isIP = iPct > 0.001 && !isIT;
  ...
  const fetchInitialData = async () => {
    ...
    try {
        ...
        if (posRes.ok) {
            ...
            const filteredPv = pvs.filter(p => !p.document_configs || p.document_configs.some(c => c.document_type === 'OV'));
  ...
  const fetchNextNumber = async (pvVal) => {
    const token = localStorage.getItem("token");
    const res = await fetch(`${API_URL}/config/pos/next-number?pv=${pvVal}&doc_type=OV`, {
  ...
  const fetchSalesOrder = async (orderId, silent = false) => {
    ...
    const headers = { Authorization: `Bearer ${token}` };
    try {
      const res = await fetch(`${API_URL}/sales/sales-orders/${targetId}`, { headers });
      const data = await res.json();
  ...
  const applyDataToState = (data) => {
    ...
    const hasP = (data.lines || []).some(l => (parseFloat(l.qty_delivered) || 0) > 0 || (parseFloat(l.qty_invoiced) || 0) > 0);
    if (hasP) {
      setIsReadOnly(true);
    }
    ...
    if (data.entity_id) {
       const token = localStorage.getItem("token");
       fetch(`${API_URL}/entities/${data.entity_id}`, { headers: { Authorization: `Bearer ${token}` }}).then(r => r.json()).then(setEntity);
    }
    ...
    try {
      ...
      setLinkManagerDoc({
        ...
        lines: linkedLines.map(l => {
          const ovLine = items.find(item => item.id === l.source_sales_line_id);
          return {
            ...l,
      ...
      console.error("[SalesOrderForm] Error opening link manager:", e);
  ...
  const handleUpdateLinkQty = async () => {
    ...
    const changes = linkManagerDoc.lines.filter(l => l.newQty !== l.originalQty);
    if (changes.length === 0) {
    ...
    try {
      ...
      for (const change of changes) {
        const endpoint = linkManagerDoc.type === 'REMITO' 
          ? `${API_URL}/sales/delivery-notes/adjust-link` 
        ...
        await fetch(endpoint, {
          ...
          body: JSON.stringify({
            ...
            source_sales_line_id: change.source_sales_line_id
          })
        });
      }
      ...
      setLinkManagerDoc(null);
      fetchSalesOrder(); // Refresh to see updated qty_delivered
    } catch (e) {
  ...
  const handleUnlinkAll = async () => {
    if (!id) return;
    if (!window.confirm("¿Está seguro de que desea desvincular TODOS los documentos de esta orden? Esto restaurará el estado pendiente de los ítems.")) return;

    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_URL}/sales/delivery-notes/unlink-all/ov/${id}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleConfirmRemito = () => {
    ...
    openNuevoRemito(id, { draft_key: draftKey });
  ...
  const handleOpenInvoiceModal = () => {
    ...
    const totalPending = items.reduce((acc, item) => acc + (parseFloat(item.qty || 0) - parseFloat(item.qty_invoiced || 0)), 0);
    ...
    items.forEach(item => {
      ...
      const pendingUnits = parseFloat(item.qty || 0) - parseFloat(item.qty_invoiced || 0);
  ...
  const searchEntities = async (q) => {
    const token = localStorage.getItem("token");
    const res = await fetch(`${API_URL}/entities/?type=client&q=${q}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return res.json();
  };
  ...
  if (loading) return <LoadingScreen message="Cargando Orden de Venta..." />;

  // Pending items aggregate
  const pendingCount = items.filter(i => parseFloat(i.qty || 0) - parseFloat(i.qty_delivered || 0) > 0).length;
  const pendingInvoicedCount = items.filter(i => parseFloat(i.qty || 0) - parseFloat(i.qty_invoiced || 0) > 0).length;

  return (
    <>
      {loadingLinkManager && <LoadingScreen message="Cargando Gestor de Vínculos..." />}
      <div className={`${s.formCard} ${isStandalone ? s.formCardStandalone : ''}`}>
              ...
              <div className={s.compactHeaderTitle}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <h1 style={{ fontSize: '15px', margin: 0, fontWeight: 900, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 8 }}>
                          {mode === 'new' ? 'Nueva Orden de Venta' : `Orden de Venta`}
                          {isReadOnly && <span className={s.readOnlyBadge}>MODO VISTA</span>}
                      </h1>
                      <div className={s.headerMeta}>
                          <span>{mode === 'new' ? 'Nueva Orden' : `OV ${joinFullNumber(pv, number)}`}</span>
                          <span>&middot;</span>
              ...
              <div className={s.headerActions}>
                  {isReadOnly ? (
                    <button 
                      ...
                      onClick={() => {
                        if (hasProgress) {
                          showToast("Atención: esta orden tiene remitos o facturas vinculadas. Edite con cuidado.", "warning");
                        }
                        setIsReadOnly(false);
                      }}
                  ...
                  <div className={s.actionGroup}>
                      <button
                        ...
                        title={!id ? "Guarde la orden para remitir" : (pendingCount === 0 ? "No hay ítems pendientes de remitir" : `Remitir Pedido — ${pendingCount} ítem(s) pendiente(s)`)}
                      ...
                      <button 
                        ...
                        title={!id ? "Guarde la orden para facturar" : (pendingInvoicedCount === 0 ? "No hay ítems pendientes de facturar" : `Facturar Pedido — ${pendingInvoicedCount} ítem(s) pendiente(s)`)}
                      ...
                      <button className={s.actionBtn} disabled={!id} onClick={() => showToast("Funcionalidad de envío por correo en desarrollo", "info")} title="Enviar por Email">
                          <Mail size={18} />
                      </button>
                  </div>
              </div>
                        ...
                        <>
                            {isReadOnly ? null : (
                                <div className={s.tableHeader}>
                                    ...
                                    <div className={s.th}>COSTO</div>
                                    <div className={s.th}>P. UNIT</div>
                                    <div className={s.th}>DTO%</div>
                                    <div className={s.th}>IVA%</div>
                                    <div className={s.th} style={{ textAlign: 'right' }}>UTILIDAD</div>
                            ...
                            <div className={s.itemsList}>
                                {items.map(item => {
                                    if (isReadOnly) {
                                        ...
                                        const utilidad = (item.cost_price > 0 && item.unit_price > 0) ? ((item.unit_price * (1 - (item.discount_pct||0)/100)) - item.cost_price) * totalQty : 0;
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                    ...
                                                    {qtyDelivered > 0 && (
                                                        <div style={{ fontSize: 10, fontWeight: 800, color: qtyDelivered >= qtyOrdered ? '#059669' : '#d97706', background: qtyDelivered >= qtyOrdered ? '#d1fae5' : '#fef3c7', padding: '4px 8px', borderRadius: 4 }}>
                                                            Remitido: {qtyDelivered} / {qtyOrdered} · Pte: {(qtyOrdered - qtyDelivered).toFixed(2)}
                                                        </div>
                                                    )}
                                                </div>
                                        ...
                                        <div key={item.id} className={s.tableRow} style={isPending && isReadOnly ? { borderLeft: '3px solid #f59e0b' } : {}}>
                                            <div style={{ padding: '4px 0', overflow: 'hidden' }}>
                                                <input 
                                                    ...
                                                    value={item.description || item.name || item.product?.name || ''} 
                                            ...
                                            <input type="number" className={s.tableInput} value={item.qty_packages !== undefined ? item.qty_packages : (item.qty || 0)} onChange={(e) => handleUpdateItem(item.id, 'qty_packages', e.target.value)} readOnly={hasProgress || isReadOnly} style={{ background: (hasProgress || isReadOnly) ? '#f8fafc' : '#fff' }} />
                                            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                                                <input type="number" className={s.tableInput} value={item.qty || 0} onChange={(e) => handleUpdateItem(item.id, 'qty', e.target.value)} />
                                            ...
                                            <div style={{ fontSize: 12, fontWeight: 900, color: 'var(--primary)', textAlign: 'right' }}>
                                                {fmtValue(((item.qty || 0) * item.unit_price * (1 - (item.discount_pct||0)/100)) * (1 + (item.vat_rate||0.21)))}
                                            </div>
          ...
          <div className={s.relationsBar}>
              ...
              <div className={s.relationCard} onClick={() => setDrawerState({ open: true, type: 'REMITO' })}>
                  ...
                  {nextLogicalAction === 'REMITO' && <button className={s.relationAction} onClick={(e) => { e.stopPropagation(); handleOpenRemitoModal(); }}>Generar</button>}
              </div>
          ...
          <div className={s.footerCompact}>
              ...
              <div className={s.footerCompactItem}>
                  <span className={s.footerCompactLabel} style={{ color: 'var(--primary)' }}>TOTAL CARTERA</span>
        ...
        <Modal open={showInvoiceModal} title="Seleccionar ítems a facturar" onClose={() => setShowInvoiceModal(false)} wide>
          <div style={{ padding: '0 24px 24px' }}>
            <p style={{ fontSize: 13, color: '#64748b', marginBottom: 20, fontWeight: 600 }}>
              ...
              Lo que no factures quedará como <strong>pendiente</strong> en la orden.
            </p>
      ...
      {linkManagerDoc && (
          ...
          >
              <div style={{ padding: '0 24px 24px' }}>
                  <div style={{ display: 'flex', gap: 24, marginBottom: 20 }}>
                      <div style={{ flex: 1 }}>
                          <p style={{ fontSize: 13, color: '#64748b', marginBottom: 12, fontWeight: 600 }}>
                              Ajustá las cantidades vinculadas a este pedido. Si reducís la cantidad a 0, el ítem se <strong>desvinculará</strong>.
                          </p>
                      </div>
                      <div style={{ background: '#f8fafc', padding: '12px 20px', borderRadius: 16, border: '1px solid #e2e8f0', minWidth: 280 }}>
                          <div style={{ fontSize: 10, fontWeight: 900, color: '#94a3b8', marginBottom: 8, letterSpacing: '0.05em' }}>PROYECCIÓN DE ESTADO</div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                              <TraceabilityStatusBadge status={status} />
                  ...
                  <div style={{ borderRadius: 16, border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: 24 }}>
                      {/* Header */}
                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 110px 110px 110px 140px', gap: 12, padding: '10px 16px', background: '#f8fafc', borderBottom: '2px solid #e2e8f0', fontSize: 10, fontWeight: 900, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', alignItems: 'center' }}>
                          <div>PRODUCTO</div>
                          <div style={{ textAlign: 'center' }}>PEDIDO OV</div>
                          <div style={{ textAlign: 'center' }}>VINCULADO</div>
                          <div style={{ textAlign: 'center' }}>ESTADO OV</div>
                      ...
                      {linkManagerDoc.lines.map((l, idx) => {
                          ...
                          return (
                              <div key={l.id} style={{ display: 'grid', gridTemplateColumns: '2fr 110px 110px 110px 140px', gap: 12, padding: '14px 16px', borderBottom: '1px solid #f1f5f9', alignItems: 'center', background: isRemoving ? '#fff1f2' : '#fff', transition: 'background 0.15s' }}>
                                  <div>
                                      <div style={{ fontSize: 13, fontWeight: 700, color: '#1e293b' }}>{l.name}</div>
                                      <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 2 }}>{l.description}</div>
                                  </div>

                                  {/* PEDIDO OV */}
                                  <div style={{ textAlign: 'center', color: '#64748b' }}>
                                      <div style={{ fontSize: 12, fontWeight: 800 }}>{l.ovQty.toFixed(2)} env.</div>
                                      <div style={{ fontSize: 10, fontWeight: 600 }}>{(l.ovQty * l.unitFactor).toFixed(1)} {l.unitLabel}</div>
                                  </div>

                                  {/* VINCULADO (Current) */}
                                  <div style={{ textAlign: 'center', color: '#24389c' }}>
                                      <div style={{ fontSize: 13, fontWeight: 900 }}>{parseFloat(l.originalQty).toFixed(2)} env.</div>
                                      <div style={{ fontSize: 10, fontWeight: 600 }}>{(l.originalQty * l.unitFactor).toFixed(1)} {l.unitLabel}</div>
                                  </div>

                                  {/* ESTADO OV (After removal/adjustment?) For now simple current status */}
                                  <div style={{ textAlign: 'center' }}>
                                      <div style={{ fontSize: 13, fontWeight: 900, color: '#059669' }}>{l.ovDelivered.toFixed(2)} env.</div>
                                      <div style={{ fontSize: 10, fontWeight: 600, color: '#059669' }}>{(l.ovDelivered * l.unitFactor).toFixed(1)} {l.unitLabel}</div>
                                  </div>
                                  ...
                                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center' }}>
                                      <input 
                                        ...
                                        max={l.originalQty}
                                        onChange={(e) => {
                                            const val = Math.min(parseFloat(l.originalQty), Math.max(0, parseFloat(e.target.value) || 0));
                                            const newLines = [...linkManagerDoc.lines];
                                            newLines[idx].newQty = val;
                                            setLinkManagerDoc({ ...linkManagerDoc, lines: newLines });
                                        }}
                                        style={{ width: 90, padding: '6px 10px', borderRadius: 10, border: '2px solid #e2e8f0', textAlign: 'center', fontWeight: 800, fontSize: 14, color: isRemoving ? '#ef4444' : '#1e293b', background: '#fff', outline: 'none' }}
                                      />
                                      <div style={{ fontSize: 9, fontWeight: 700, color: '#64748b' }}>= {(l.newQty * l.unitFactor).toFixed(2)} {l.unitLabel}</div>
                                  </div>
                              </div>
                          );
                      })}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 16 }}>
                    <button onClick={() => setLinkManagerDoc(null)} style={{ padding: '10px 22px', borderRadius: 12, border: '1.5px solid #e2e8f0', background: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>Cancelar</button>
                    <button
                      onClick={handleUpdateLinkQty}
                      style={{ padding: '10px 24px', borderRadius: 12, border: 'none', background: '#24389c', color: 'white', fontWeight: 800, cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}
                    >
        ...
        {mode === 'edit' && (
            <>
                <ManualLinkDeliveryNoteModal 
                    open={isManualLinkModalOpen} 
                    onClose={() => setIsManualLinkModalOpen(false)}
                    orderId={id}
                    entityId={entity?.id}
                    orderLines={items}
                    onLinked={fetchSalesOrder}
                />

                <ManualLinkInvoiceModal 
                    open={isManualInvoiceLinkOpen} 
                    onClose={() => setIsManualInvoiceLinkOpen(false)}
                    orderId={id}
                    entityId={entity?.id}
                    onLinked={fetchSalesOrder}
                />
