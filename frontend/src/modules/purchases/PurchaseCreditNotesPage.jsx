...
export default function PurchaseCreditNotesPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("invoice-changed", handleRefresh); // Generic event
  ...
  const fetchInvoices = async () => {
    try {
      ...
      showToast("Error al cargar notas de crédito de compra", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = () => {
    openWindow(
      ...
      {
        title: "Nueva Nota de Crédito (Compra)",
        width: 1100,
        height: 700,
      }
    );
  ...
  const handleEdit = (inv) => {
    openWindow(
      ...
      {
        title: `Nota de Crédito ${inv.number}`,
        width: 1100,
        height: 700,
      }
    );
  ...
  return (
    <div className={s.pageLayout}>
      <ContentHeader 
        ...
        subtitle="Bonificaciones y devoluciones de proveedores"
        ...
        actions={
            <Button variant="primary" className={s.primaryCta} onClick={handleCreate} style={{ height: 44, padding: '0 24px' }}>
                <Plus size={18} /> Cargar Nota de Crédito
            </Button>
        }
      />
                  ...
                  <td colSpan="7" style={{ padding: 80, textAlign: 'center' }}>
                    <div className={s.empty} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                      <div className={s.emptyTitle} style={{ fontSize: 18, fontWeight: 700, color: '#1e293b' }}>
                        {hasActiveFilters ? "No hay resultados" : "Sin notas de crédito"}
                      </div>
                    </div>
                  </td>
                </tr>
