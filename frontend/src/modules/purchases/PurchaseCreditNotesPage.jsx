...
import { openNuevaNotaCreditoCompra, openEditNotaCreditoCompra } from '../../utils/openStandaloneWindow';
...
export default function PurchaseCreditNotesPage() {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const filteredDocs = data.filter((d) => d.doc_type === "PURCHASE_CREDIT_NOTE");
  ...
  const handleOpenNew = () => {
    openNuevaNotaCreditoCompra();
  };

  const handleOpenDetail = (id, number) => {
    openEditNotaCreditoCompra(id, { title: `NC Compra ${number}` });
  };
  ...
  const stats = useMemo(() => {
    ...
    const appliedDocs = invoices.filter(inv => inv.status === 'APPLIED' || inv.status === 'PAID');
    const openDocs = invoices.filter(inv => inv.status === 'OPEN' || inv.status === 'PARTIAL');
  ...
  const kpis = [
    ...
    {
      title: "Aplicadas Parcial",
      value: stats.openCount,
      subtitle: `Notas con saldo`,
      icon: Clock,
      type: "warning"
    },
    {
      title: "Aplicadas Total",
    ...
    filtersArea: showFilters && (
      <div className={s.compactFiltersRow}>
          <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className={s.filterSelect}>
            ...
            <option value="PAID">✅ Aplicada / Pagada</option>
  ...
  const table = {
    ...
    body: loading ? (
        <TableRowSkeleton rows={7} cols={7} />
    ) : error ? (
      ...
      <tr>
        <td colSpan="7">
          <EmptyState 
              icon={FileText} 
              title={hasActiveFilters ? "Sin resultados" : "Sin notas de crédito de compra"}
              description={hasActiveFilters ? "Ajustá los filtros para encontrar lo que buscás." : "Comenzá creando una nota de crédito de proveedor."}
              actionLabel={!hasActiveFilters ? "Crear Nota de Crédito" : null}
        ...
        <tr key={inv.id} className={`${s.row} ${selectedInvoices.includes(inv.id) ? s.rowSelected : ""}`} onClick={() => handleOpenDetail(inv.id, inv.number)}>
          ...
          <td className={s.td} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              ...
              <button onClick={() => handleOpenDetail(inv.id, inv.number)} style={{ all: 'unset', cursor: 'pointer', color: 'var(--accent-indigo)' }} title="Ver Nota de Crédito">
  ...
  return (
    <>
      <DocumentListPage 
        ...
        breadcrumbs={[{ label: 'Compras' }, { label: 'Notas de Crédito' }]}
        kpis={kpis}
        toolbar={toolbar}
        table={table}
